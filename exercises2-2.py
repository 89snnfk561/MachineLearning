...
# init
nb_samples = 50
X, Y = make_classification(n_samples=nb_samples, n_features=2, n_clusters_per_class=1, n_classes=3, n_informative=2, n_redundant=0)
...
def show_dataset(X, Y):
    ...
    for i in range(nb_samples):
        if Y[i] == 0:
            ax.scatter(X[i, 0], X[i, 1], marker='o', color='r', s = 40)
        elif Y[i] == 1:
            ax.scatter(X[i, 0], X[i, 1], marker='^', color='b', s = 40)
        else:
            ax.scatter(X[i, 0], X[i, 1], marker='^', color='g', s = 40)
